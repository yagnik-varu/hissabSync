...
- **E2E Testing Insight (Archive Room)**: Discovered and fixed a critical gap where the `Archive Room` status transition was never implemented. We added `RoomNotArchivedGuard` and applied it to state-mutating endpoints across `Treasury`, `Expense`, and `Room` modules. We also wrote `4-archive-room.e2e-spec.ts`. This was better caught now via comprehensive e2e testing because cross-module side effects (like Room status dictating Treasury constraints) are inherently missed when modules are developed and unit-tested in isolation (e.g., Treasury tests assume a mocked ACTIVE room). If this had shipped, users could archive a room but financial ledgers would remain unfrozen, violating core compliance constraints.
